const crewMembers = ['River', 'Zoe', 'Wash', 'Mal', 'Book', 'Jayne'];
...
const indexOfName = name => crewMembers.indexOf(name);

// Implement these functions to answer the questions below
const numberOfQuotes = name => quotes[indexOfName(name)].length;

const whoSaidThis = (quote) => {
  let indexOfPerson;
  quotes.forEach((quotesOfPerson, index) => {
    if (quotesOfPerson.includes(quote)) {
      indexOfPerson = index;
    }
  });
  return crewMembers[indexOfPerson];
};

const longestQuote = (name) => {
  let currentLongestQuote = '';
  quotes[indexOfName(name)].forEach((quote) => {
    if (quote.length > currentLongestQuote.length) {
      currentLongestQuote = quote;
    }
  });
  return currentLongestQuote;
};

const averageQuoteLength = (name) => {
  const quotesOfPerson = quotes[indexOfName(name)];
  const quoteLengthSum = quotesOfPerson.reduce((acc, curr) => acc + curr.length, 0);
  const quoteLengthAverage = quoteLengthSum / quotesOfPerson.length;
  return (quoteLengthAverage).toFixed();
};
...
const washQuote = 'I am a leaf on the wind. Watch how I soar.';
console.log(`${whoSaidThis(washQuote)} says: ${washQuote}`);

// Jayne says: Let's be bad guys.
const jayneQuote = "Let's be bad guys.";
